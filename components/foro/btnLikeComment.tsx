...
function BtnLikeComment(props: any) {
    ...
    const [countComments, setCountComments] = useState(
        props.item.hasOwnProperty('commentsList') && props.item.commentsList.length > 0 ? props.item.commentsList[0].count : 0
    );
    ...
    /*const setApproved = () => {
        ...
        if(id_user === ''){
            ...
            let pts = [...points];
            pts.includes(id_user)? pts.splice(pts.indexOf(id_user)):(pts.push(id_user));
            setPoints(pts);
            props.updatePointsComment(props.item._id, pts.includes(id_user));
        }
    };*/
    ...
    const setApproved = (type: any) => {
        ...
        let pts = [...points];
        let pip = [];
        const appr = pts.filter((elemt: any) => elemt.user === id_user);

        if (appr.length > 0) {
            const indx = pts.findIndex((el: any) => el.user === id_user);
        ...
        setPoints(pts);
        props.updatePointsComment(props.item._id, pip);
        }
    };
            ...
            <div className="cardHeader" style={{ margin: "15px 0px 10px 0px" }}>
                <div className={styles.likeComment}>
                    <Tooltip title="Punto positivo" arrow>
                        ...
                        >
                            {getUserPoints(id_user, "approve").length > 0 ?
                    ...
                    <Tooltip title="Punto negativo" arrow>
                        <div
                            ...
                            className={styles.divLikes}
                        >
                            {getUserPoints(id_user, "disapprove").length > 0 ?
...
export default connect(mapStateToProps, mapDispatchToProps)(BtnLikeComment);
